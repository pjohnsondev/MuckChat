...
public enum MuckServer {
	...
	synchronized void startKryo(KryoServerConfig config) throws IOException {
		...
		// Make necessary migrations to set up database (their not really migrations but
		// what can you do)
		...
		/**
		 * This is a temporary call to help populate the server with an initial test
		 * user If you are seeing errors in playermanagertest or from the server
		 * regarding COLUMN not existing, please delete the muckdb folder and the testdb
		 * folder from the server The next time you start the server it will
		 * automatically create a new database with the latest structure
		 */
		...
		/**
		 * Listens for a userMessage class coming from the client. Calls a worker to
		 * handle storing the message in the chat log (not done yet).
		 */
		 ...
		 * Acts as a signal to tell chatCreateTable to create a new chat log with
		 * specified name.
		 */
